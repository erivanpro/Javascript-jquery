...
function createField(theField, type)
{
	...
	switch (theField.FLD_type)
	{
		...
		case 'liste de textes':
			newField
				.append('<div class="textListComponent"><div class="content"><label class="lblInput"></label></div><input type="text"></div>');
			newField
				.css('height', '180px');
			break;
		case 'image':
			newField
				.append('<img class="imageComponent" src="images/imageDef.png">');
			newField
				.css('height', '180px');
			break;
	}
		  ...
		  .css	(
			  {
			    left: geometry.left,
			    top: geometry.top,
			    width: geometry.labelSize + geometry.componentSize 
			  }
			);
		newField
			.find ('label:first')
